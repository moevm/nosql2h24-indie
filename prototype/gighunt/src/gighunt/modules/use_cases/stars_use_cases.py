...
class StarsCollections(Enum):
        STARSTOUSER = "StarsToUser"
        STARSTOGROUP = "StarsToGroup"
        STARSTOANNOUNCEMENT = "StarsToAnnouncement"

class StarsUseCases(BaseEdgeUseCases):
    def __init__(self, db_client: ArangoDBClient, graph: Graph, vertex_names: dict):
        super().__init__(db_client, graph, vertex_names)
        self.edge_collection_names = StarsCollections
