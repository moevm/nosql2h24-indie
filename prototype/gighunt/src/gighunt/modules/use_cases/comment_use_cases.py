...
class CommentCollections(Enum):
    COMMENTTOANNOUNCEMENT = "CommentToAnnouncement"

class CommentUseCases(BaseEdgeUseCases):
    def __init__(self, db_client: ArangoDBClient, graph: Graph,vertex_names:dict):
        super().__init__(db_client, graph, vertex_names)
        self.edge_collection_names = CommentCollections
